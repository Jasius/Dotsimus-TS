import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, SlashCommandBuilder, time, TimestampStyles } from 'discord.js';
...
export default class AboutCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            options: new SlashCommandBuilder()
                .addSubcommand((subcommand) => subcommand.setName('me').setDescription('Shows general information about bot and its commands.'))
                .addSubcommand((subcommand) => subcommand.setName('uptime').setDescription('Shows how long bot stayed up since the last restart.'))
                .addSubcommand((subcommand) => subcommand.setName('ping').setDescription('Shows bots latency.'))
                .addSubcommand((subcommand) => subcommand.setName('restart').setDescription('⚠️ [Owner command] Restarts the bot.'))
    ...
    async executeMe(interaction) {
        ...
        const embed = new EmbedBuilder({ color: 0xffbd2e })
            ...
            .addFields({ name: 'Servers', value: guilds.size.toLocaleString(), inline: true }, {
            name: 'Users',
    ...
    async executeUptime(interaction) {
    ...
    async executeRestart(interaction) {
    ...
    async executeSubmitReview(interaction) {
        const row = new ActionRowBuilder().addComponents(new ButtonBuilder()
