...
export default class MessageCreateEvent extends Event {
	...
	async execute(message: Message) {
		...
		if (message.channel.type === ChannelType.DM) {
			...
			if (message.author.id === owner.id && message.reference?.messageId) {
				const ref = await message.channel.messages.fetch(message.reference.messageId);

				try {
					const refUser = this.client.users.cache.get(ref.content.split(/ +/g)[0]);
		...
		const serverConfig = await this.client.utils.saveServerConfig(message.guild);
		...
		for (const watchedKeywords of serverWatchedKeywords) {
			...
			const trackingEmbed = new EmbedBuilder()
				...
				.setColor(this.client.utils.getRandomColor(message.member.displayName));
		...
		if (toxicity >= 0.85 || (insult >= 0.95 && user.isNew) || toxicity >= 0.85 || combined >= 0.85) {
			...
			async function getLatestUserMessages(userId: string) {
				const messages: Collection<string, Message> = await message.channel.messages.fetch({
					limit: 30,
				});
