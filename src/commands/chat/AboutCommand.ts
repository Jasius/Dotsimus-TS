...
export default class AboutCommand extends Command {
	...
	async executeMe(interaction: ChatInputCommandInteraction): Promise<CommandResponse> {
		...
		const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
			...
			new ButtonBuilder().setLabel('Join Support').setURL('https://discord.gg/XAFXecKFRG').setStyle(ButtonStyle.Link),
		);
	...
	async executeSubmitReview(interaction: ChatInputCommandInteraction): Promise<CommandResponse> {
		...
		return interaction.reply({
			content:
				"Reviews significantly help to amplify the voice of the users and help to find focus areas for further Dotsimus development, if you'd like to leave some feedback please do so on [top.gg](https://top.gg/bot/731190736996794420#reviews). It's highly appreciated!",
			components: [row],
			ephemeral: true,
		});
	}
